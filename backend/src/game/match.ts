...
class Match {
	static setup(app: FastifyInstance) {
		...
		app.post('/api/match/create', { preHandler: authGuard }, Match.createMatch); //use case: player invited to play by chat 
	...
	static async createMatch(req: FastifyRequest, rep: FastifyReply) {
		const { p1Id, p2Id } = req.body as { p1Id: number, p2Id: number };

		const [p1Playing] = await db.select().from(matches).where(and(eq(matches.status, "ongoing"), or(
			eq(matches.player1Id, p1Id),
			eq(matches.player2Id, p1Id),
		)));
		const [p2Playing] = await db.select().from(matches).where(and(eq(matches.status, "ongoing"), or(
			eq(matches.player1Id, p2Id),
			eq(matches.player2Id, p2Id),
		)));
